...
class WagtailModeltranslationAdminPageForm(WagtailAdminPageForm):
    ...
    def clean(self):
        ...
        for isocode, description in settings.LANGUAGES:
            slugs_to_check["slug_%s" %
                           isocode] = cleaned_data["slug_%s" % isocode]

        for slugfield, des_error in _validate_slugs(
                self.instance,
...
# We here inherit the original CopyForm, we could define a new one instead
# (as at today we redefine all its methods)
class NewCopyForm(CopyForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Yeah, call CopyForm not NewCopyForm
        ...
        # )
        for isocode, description in settings.LANGUAGES:
            self.fields['new_title_%s' % isocode] = forms.CharField(
                initial=getattr(self.page, "title_%s" % isocode),
                label=_("New title") + (" [%s]" % isocode)
            )
        ...
        # )
        for isocode, description in settings.LANGUAGES:
            self.fields['new_slug_%s' % isocode] = forms.CharField(
                initial=getattr(self.page, "slug_%s" % isocode),
                label=_("New slug") + (" [%s]" % isocode)
            )
        ...
        if can_publish:
            ...
            if pages_to_publish_count > 0:
                # In the specific case that there are no subpages, customise
    ...
    def clean(self):
        ...
        # New parent page given in form or parent of source, if parent_page is
        # empty
        ...
        for isocode, description in settings.LANGUAGES:
            ...
            slugs_to_check["slug_%s" %
                           isocode] = cleaned_data.get(formfieldname) or ""
