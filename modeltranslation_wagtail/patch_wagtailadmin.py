...
class WagtailTranslator(object):
    ...
    def __init__(self, model):
        ...
        # Set the required of the translated fields that were required on the
        # original field
    ...
    def _fetch_defined_tabs(defined_class):
        ...
        # Check for common tabs
            ...
            if hasattr(defined_class, 'edit_handler'):
                ...
                # Find all class members that could be tab panels like
                # content_panels, settings_panels, promote_panels and any other
                # custom defined panels
                tab_panels = inspect.getmembers(
                    defined_class, lambda x: isinstance(x, list))

                k = -1
                for objlist in defined_class.edit_handler.children:
                    k += 1

                    # Try to understand if this tab_panel was defined as model
                    # attribute, so we can recycle its name and update its
                    # value
                    for tab_panel_name, tab_panel in tab_panels:
                        ...
                        # It happens if a custom tab panel is defined as list
                        # onto the edit_handler directly
        ...
        if fieldpanel.field_name in tr_fields:
            for lang in settings.LANGUAGES:
                ...
                translated_fieldpanels.append(
                    FieldPanel(translated_field_name,
                               classname=classes, widget=fieldpanel.widget)
                )
        ...
        if imagechooser.field_name in tr_fields:
            for lang in settings.LANGUAGES:
                ...
                translated_imagechoosers.append(
                    ImageChooserPanel(translated_field_name))
        else:
            return [imagechooser]
        ...
        if fieldpanel.field_name in tr_fields:
            for lang in settings.LANGUAGES:
                ...
                translated_field_name = build_localized_fieldname(
                    fieldpanel.field_name, lang[0])
                translated_fieldpanels.append(
                    StreamFieldPanel(translated_field_name))
        else:
            return [fieldpanel]
...
def _new_move(self, target, pos=None):
    ...
    # Log
    logger.info("Page moved: \"%s\" id=%d path=%s",
                self.title, self.id, new_url_path)
    ...
    for lang in settings.LANGUAGES:
        if parent:
            ...
            if hasattr(self, 'url_path_' + lang[0]):
                setattr(self, 'url_path_' +
                        lang[0], parent_url_path + tr_slug + '/')
            else:
            ...
            if len(slug) and model is not Page:
                if model in WagtailTranslator._patched_models:
                    field_name = '{0}__{1}'.format(
                        model._meta.model_name, current_slug)
                else:
